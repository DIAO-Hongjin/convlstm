...
class ConvLSTMCell(nn.Module):
    ...
    def __init__(self, shape, in_channels, out_channels, kernel_size):
        ...
        self.conv1 = nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, 1, self.padding, 1, 1, False)
        self.conv2 = nn.Conv2d(self.out_channels, self.out_channels, self.kernel_size, 1, self.padding, 1, 1, False)
        self.conv3 = nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, 1, self.padding, 1, 1, False)
        self.conv4 = nn.Conv2d(self.out_channels, self.out_channels, self.kernel_size, 1, self.padding, 1, 1, False)
        self.conv5 = nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, 1, self.padding, 1, 1, False)
        self.conv6 = nn.Conv2d(self.out_channels, self.out_channels, self.kernel_size, 1, self.padding, 1, 1, False)
        self.conv7 = nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, 1, self.padding, 1, 1, False)
        self.conv8 = nn.Conv2d(self.out_channels, self.out_channels, self.kernel_size, 1, self.padding, 1, 1, False)
...
class ConvLSTMNet(nn.Module):
    """
    ConvLSTM网络（输入--ConvLSTM_1--ConvLSTM_2--...--ConvLSTM_n--输出）
    """
    ...
    def __init__(self, shape, in_channels, hidden_channels, kernel_size):
        ...
        :param hidden_channels: 每一ConvLSTM隐藏层的输出通道数
        :param kernel_size: 卷积核的尺寸
        """
        ...
        for i in range(self.layers_num):
            layers_list.append(
                ConvLSTMCell(self.shape, self.input_channels[i], self.hidden_channels[i], self.kernel_size).to(device)
            )
    ...
    def forward(self, in_tensor, last_hidden_list=None):
        ...
        for i in range(self.layers_num):
            ...
            for j in range(seq_len):
                hidden_state, cell_out = self.layers_list[i](cur_input[:, j, :, :, :], hidden_state, cell_out)
...
class EncoderForecasterModel(nn.Module):
    ...
    def __init__(self, out_seq_len, shape, in_channels, hidden_channels, kernel_size):
        ...
        self.encoder = ConvLSTMNet(self.shape, self.in_channels, self.hidden_channels, self.kernel_size)
        self.forecaster = ConvLSTMNet(self.shape, self.in_channels, self.hidden_channels, self.kernel_size)
...
class NonBalanceLoss(nn.Module):
    def __init__(self):
        super(NonBalanceLoss, self).__init__()

    def forward(self, pred, target, mask):
        weights = torch.ones(pred.size())
        weights = weights.to(device)
        weights = weights*mask

        mse = torch.mean(torch.sum(weights*torch.pow(pred-target, 2), dim=(2, 3, 4)))
        mae = torch.mean(torch.sum(weights*torch.abs(pred-target), dim=(2, 3, 4)))

        loss = mse*0.5+mae*0.5
...
logger = Logger(log_name='non_balance_convlstm_hko_7_log.txt', log_level=1, log_object="hko-7").get_log()  # 定义日志
...
if __name__ == '__main__':
    ...
    for iteration in range(MAX_ITERATION):
        ...
        if(iteration+1)%VALID_ITERATION == 0:
            count += 1
            with torch.no_grad():
                valid_env = HKOBenchmarkEnv(pd_path=cfg.HKO_PD.RAINY_VALID, save_dir="non_balance_convlstm_hko7_valid", mode="fixed")
                ...
                if valid_loss > best_valid_running_loss:
                    ...
                    no_better_validation_step = 0
                    torch.save(net, 'non_balance_convlstm_hko_7_best_model.pkl')

    logger.info("训练完成！")

    # 加载验证集上效果最好的模型
    if torch.cuda.is_available():
        final_net = torch.load('non_balance_convlstm_hko_7_best_model.pkl')
    else:
        final_net = torch.load('non_balance_convlstm_hko_7_best_model.pkl', map_location=lambda storage, loc: storage)

    # 检查模型在测试集中的表现

    with torch.no_grad():
        test_env = HKOBenchmarkEnv(pd_path=cfg.HKO_PD.RAINY_TEST, save_dir="non_balance_convlstm_hko7_test", mode="fixed")
