...
logger = Logger(log_name='dconvlstm_hko_7_log.txt', log_level=1, log_object="hko-7").get_log()  # 定义日志
...
if __name__ == '__main__':
    ...
    for iteration in range(MAX_ITERATION):
        ...
        if(iteration+1)%VALID_ITERATION == 0:
            count += 1
            with torch.no_grad():
                valid_env = HKOBenchmarkEnv(pd_path=cfg.HKO_PD.RAINY_VALID, save_dir="dconvlstm_hko7_valid", mode="fixed")
                ...
                if valid_loss > best_valid_running_loss:
                    ...
                    no_better_validation_step = 0
                    torch.save(net, 'dconvlstm_hko_7_best_model.pkl')

    logger.info("训练完成！")

    # 加载验证集上效果最好的模型
    if torch.cuda.is_available():
        final_net = torch.load('dconvlstm_hko_7_best_model.pkl')
    else:
        final_net = torch.load('dconvlstm_hko_7_best_model.pkl', map_location=lambda storage, loc: storage)

    # 检查模型在测试集中的表现

    with torch.no_grad():
        test_env = HKOBenchmarkEnv(pd_path=cfg.HKO_PD.RAINY_TEST, save_dir="dconvlstm_hko7_test", mode="fixed")
