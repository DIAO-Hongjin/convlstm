...
def plot_image(index, in_seq_len, out_seq_len,
    ...
    for i in range(item_num):
        ...
        pyplot.savefig(
            "convlstm_k5_moving_mnist_test_result/%0.4dth_test_sample.png" % (index*item_num+i+1),
            dpi=128
        )
        pyplot.show()
...
logger = Logger(log_name='convlstm_k5_moving_mnist_log.txt', log_level=1, log_object="moving-mnist").get_log()  # 定义日志
...
if __name__ == '__main__':
    ...
    for epoch in range(MAX_EPOCH):
        ...
        if cur_valid_running_loss > best_valid_running_loss:
            ...
            no_better_validation_step = 0
            torch.save(net, 'convlstm_k5_moving_mnist_best_model.pkl')

    logger.info("训练完成！")

    # 加载验证集上效果最好的模型
    if torch.cuda.is_available():
        final_net = torch.load('convlstm_k5_moving_mnist_best_model.pkl')
    else:
        final_net = torch.load('convlstm_k5_moving_mnist_best_model.pkl', map_location=lambda storage, loc: storage)
